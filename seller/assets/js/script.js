...
function show_preview(e, f) {
  // file preview improved to avoid reliance on global `event`
  var input = document.getElementById(f);
  if (validatefile(f)) {
    var reader = new FileReader();
    reader.onload = function (ev) {
      var output = document.getElementById(e);
      output.src = ev.target.result;
    };
...
function validatefile(e) {
  ...
  // Try to validate by file name first (works on most browsers).
  var fileName = "";
  if (fileInput.value) {
  ...
  // Additional MIME type check (some browsers report different values)
  if (fileInput.files && fileInput.files[0]) {
    var type = fileInput.files[0].type;
    if (type && !/(image\/(jpeg|jpg|png))/i.test(type)) {
      fileInput.value = "";
      return 0;
    }
  }
...
function getstatelist() {
  ...
  control.html("fscb", '<option value="">Select State</option>');
  control.html("fscb2", '<option value="">Select City</option>');
  control.html("fscb3", '<option value="">Select Pincode</option>');
  if (country == "") return; // nothing to load
  $.ajax({
