...
{
	$sql = "select * from product where category='$p_id' and subcat='$s_id' and isappp='1' and status='1' order by id desc";
...
{
	$sql = "select product.*,filters.name from product,filters where product.category='$p_id' and product.subcat='$s_id' and filters.subcat=product.subcat and filters.name='$f_id' and product.isappp='1' and product.status='1' order by product.id desc";
	$res = mysqli_query($con, $sql);
	$product_arr = array();
	while ($row = mysqli_fetch_assoc($res)) {
		$product_arr[] = $row;
...
{
	$sql = "select product.*,filters.name,product_subfilters.subfilter from product,filters,product_subfilters where product.category='$p_id' and product.subcat='$s_id' and filters.subcat=product.subcat and filters.name='$f_id' and product.isappp='1' and product.status='1' and product_subfilters.subfilter='$sf_id' and product_subfilters.pid=product.id order by product.id desc";
	$res = mysqli_query($con, $sql);
	$product_arr = array();
	while ($row = mysqli_fetch_assoc($res)) {
		$product_arr[] = $row;
...
{
	// Only return product details for admin-approved (released) items.
	$sql = "select product.*,sellers.f_name from product,sellers where product.id='$id' and product.added_by=sellers.id and product.isappp='1' and product.status='1'";
	$res = mysqli_query($con, $sql);
...
{
	$sql = "select * from product where status='1' and bs='1' and isappp='1' order by id desc limit 6";
	$res = mysqli_query($con, $sql);
	$product_arr = array();
	while ($row = mysqli_fetch_assoc($res)) {
		$product_arr[] = $row;
...
{
	$sql = "select * from product where status='1' and bs='1' and isappp='1' order by id desc";
	$res = mysqli_query($con, $sql);
	$product_arr = array();
	while ($row = mysqli_fetch_assoc($res)) {
		$product_arr[] = $row;
...
{
	$sql = "select * from product where status='1' and isappp='1' order by id desc limit 12";
	$res = mysqli_query($con, $sql);
	$product_arr = array();
	while ($row = mysqli_fetch_assoc($res)) {
		$product_arr[] = $row;
...
function search_product($con, $search_name, $cat, $scat = '', $filter = '', $sfilter = '')
{
	...
	if ($filter == '' && $sfilter == '') {
		...
		$sql = "select * from product where status='1' and isappp='1' $gg and(product_name like '%$search_name%' or description like '%$search_name%') $add";
	} else {
		if ($filter != '' && $sfilter == '') {
			$sql = "select product.*,p_filter.fid from product,p_filter where product.status='1' and product.isappp='1' and product.cat_id='$cat' and product.scat_id='$scat' and(product.product_name like '%$search_name%' or product.description like '%$search_name%') and p_filter.pid=product.id and p_filter.fid='$filter'";
		} else {
			$sql = "select product.*,p_filter.fid,p_sfilter.sfid from product,p_filter,p_sfilter where product.status='1' and product.isappp='1' and product.cat_id='$cat' and product.scat_id='$scat' and(product.product_name like '%$search_name%' or product.description like '%$search_name%') and p_filter.pid=product.id and p_sfilter.pid=product.id and p_filter.fid='$filter' and p_sfilter.sfid='$sfilter'";
		}
	}
	$res = mysqli_query($con, $sql);
	$product_arr = array();
...
function authorise($con)
{
	...
	// Allow seller to access their dashboard landing / profile pages even if not yet approved.
	$allowedUnauthPages = [
		'index.php',
		'complete_profile.php',
		'complete_profile_simple.php',
		'edit_profile.php',
		'logout.php',
	];
	$currentPage = basename($_SERVER['PHP_SELF']);
	if (in_array($currentPage, $allowedUnauthPages)) {
		return;
	}

	$seller_id = $_SESSION['SELLER_ID'];
	$row = mysqli_fetch_assoc(mysqli_query($con, "select isapp from sellers where id='$seller_id'"));
	if (!$row || $row['isapp'] != 1) {
		// Seller account is not approved yet; send them back to the dashboard to see the status message.
		redirect('index.php');
	}
}
